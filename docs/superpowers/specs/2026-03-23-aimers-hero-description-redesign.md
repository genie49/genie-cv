...
| 파일 | 변경 내용 |
|------|----------|
| `apps/client/src/components/KimproHero.tsx` | SVG 전체 재설계. `Node` 컴포넌트에 `"orange"` 색상 추가 (additive, 다른 hero에 영향 없음) |
| `data/projects.json` | kimpro의 `description` 필드 교체 (`\n\n`으로 4문단 구분) |
| `apps/client/src/pages/ProjectDetailPage.tsx` | description 렌더링을 `\n\n` 기준으로 `<p>` 분리 (레이아웃/구조 변경 없음) |
...
- 내부 구성:
  ...
  - **서브에이전트 4개** — Insight, Campaign, Content, Recruit (작은 흰색 노드, violet border). 실제로는 5명(+Contract Manager)이지만 도식 공간 제약으로 4개만 표시. description에서 5명 전체를 언급하여 보완.
...
### 노드 밀도 참고
기존 도식은 ~8개 노드. 새 도식은 ~20+개 노드로 밀도가 크게 증가함. 줌 인터랙션으로 보완하되, 라벨 약어 사용(예: "Notification WK", "PG")과 보조 노드의 폰트 축소(5-6px)로 가독성 확보. 정확한 좌표는 구현 시 브라우저 mockup 기준으로 조정.

### 기존 패턴 준수 사항
- 다른 hero와 동일한 viewBox `640 × 280`
- `Node`, `Arrow` 컴포넌트 재사용. `Node`의 color union에 `"orange"` 추가 필요 (additive 변경, 기존 hero에 영향 없음)
...
- ProjectDetailPage, ProjectCard 전체 레이아웃/구조 (description 렌더링 로직만 소폭 수정)
- features 카드 내용
- notes (개발 노트)
