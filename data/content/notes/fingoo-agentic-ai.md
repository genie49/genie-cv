...
| LangGraph | 저수준 그래프 API, 체크포인트/interrupt 내장 | — |

LangGraph를 선택한 결정적 이유는 **미들웨어 체인**과 **interrupt 메커니즘**입니다. 핀구는 10개의 미들웨어로 에이전트 행동을 세밀하게 제어하고, Human-in-the-Loop으로 차트 조작 시 사용자 확인을 거칩니다. CrewAI나 AutoGen에서는 이 수준의 제어가 불가능했습니다.
...
- **프레임워크 선택은 제어 수준의 문제**: CrewAI의 편리함보다 LangGraph의 세밀한 제어를 선택한 것은, 금융 도메인에서 에이전트 행동의 예측 가능성이 개발 편의성보다 중요하기 때문
