# 만능 AI? No. 전문가 팀이 더 강력하다

처음에는 하나의 에이전트에 모든 도구를 넣었습니다. 결과는? 도구가 많아질수록 잘못된 도구를 선택하고, 컨텍스트가 폭발하고, 응답 품질이 떨어졌습니다. 이것저것 시키면 다 대충 하는 만능 AI 대신, 각자 전문 분야가 있는 에이전트 팀을 만들고 팀장(Supervisor)이 지휘하는 구조로 전환했습니다. 그 설계 과정을 정리합니다.
