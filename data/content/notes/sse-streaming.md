...
```mermaid
flowchart TB
    Stream[LangGraph agent.stream] --> Check{메시지 타입}
...
LangGraph의 `streamMode: "messages"`는 토큰을 하나씩 전송합니다. RAG 도구 호출 결과(인용 정보)는 스트리밍이 완료된 후에야 최종 확정됩니다. 토큰 스트리밍 중에는 어떤 인용이 사용될지 모릅니다.
