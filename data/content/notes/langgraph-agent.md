...
17줄의 코드로 구성된 LangGraph ReAct 에이전트가 어떻게 조건부 도구 호출, rate limiting, 에러 분류를 통해 프로덕션 환경에서 안정적으로 동작하는지 정리합니다.
...
`createReactAgent`가 ReAct 루프(추론 → 행동 → 관찰 → 추론...)를 자동으로 구성합니다. 프레임워크가 제공하는 추상화를 최대한 활용하여 코드를 최소화했습니다.
...
- **간결함이 곧 안정성**: 17줄 에이전트 + 1개 도구로 충분한 곳에 미들웨어 스택을 도입하면 오버엔지니어링. 도메인 복잡도에 맞는 아키텍처 선택이 핵심
...
- **프레임워크 활용의 균형**: `createReactAgent` 한 줄이 ReAct 루프를 자동 구성. 직접 구현하면 수십 줄의 루프 로직이 필요. 프레임워크가 제공하는 추상화를 신뢰할 수 있다면 활용
