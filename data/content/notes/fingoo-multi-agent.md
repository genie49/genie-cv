...
```
사용자 질문
  → Supervisor 에이전트 (질문 분류·라우팅)
  → 금융 분석 에이전트 / 시장 분석 에이전트 / 뉴스 검색 에이전트
  → 종합 응답 생성
```
...
```
[금융 데이터 수집 중...] → [지표 분석 중...] → [차트 생성 중...] → 응답 시작
```
...
| Tool Call | 프론트엔드 액션 |
|---|---|
| `predict_indicator` | 차트 컴포넌트 마운트 + 데이터 로드 |
| `analyze_economy` | 경제 분석 패널 표시 |
| `add_axis` | 기존 차트에 Y축 추가 |
| `split_screen` | 레이아웃을 2분할로 변경 |
| `draw_metadata` | 차트에 메타데이터 오버레이 |
